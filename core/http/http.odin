...
header_parse :: proc(headers: ^Headers, line: string, clone_value := true, allocator := context.allocator) -> (key: string, ok: bool) #no_bounds_check {
	...
	// TODO: we need to store multiple headers anyway, we can defer this host and content-length
	// check to when we have all the headers.
	...
	headers_set(headers, key, clone_value ? strings.clone(value, allocator) : value)
