...
Has_Body :: struct {
	...
	_scanner: ^Scanner,
	_max_length: int,
	body_allocator: runtime.Allocator,
}
...
body :: proc(sub: ^Has_Body, max_length: int, user_data: rawptr, cb: Body_Callback) {
	...
	if sub._body.cbs.cb == nil {
		...
		context.allocator = sub.body_allocator
		_, err := append(&sub._body.cbs.others, _Cb{
			cb        = cb,
			user_data = user_data,
		})
		if err != nil {
			cb(user_data, nil, .Already_Consumed)
		}
	}
}
...
_bufio_error_to_body_error :: proc(e: bufio.Scanner_Error, loc := #caller_location) -> Body_Error {
	...
	case io.Error:
		...
		case .None:
			return .None
		case .EOF, .Unexpected_EOF, .Closed:
			return .EOF
		case .Unknown:
			return .Unknown
		case .No_Progress:
		...
		case .Empty, .Short_Write, .Buffer_Full, .Short_Buffer,
			 ...
			 .Invalid_Unread, .Negative_Write, .Negative_Count, .No_Size, .Permission_Denied:
			log.error(e, location=loc)
			return .Corrupted_State
		case:
			return .Unknown
...
_body_chunked :: proc(sub: ^Has_Body, max_length: int = -1) {
	...
	on_scan :: proc(sub: ^Has_Body, size_line: string, err: bufio.Scanner_Error) {
		size_line := size_line

		if err != nil {
			_body_do_cbs(sub, nil, _bufio_error_to_body_error(err))
			return
		}

		sub._max_length -= len(size_line)
		...
		if size == 0 {
			scanner_scan(sub._scanner, sub, on_scan_trailer)
			return
		}

		if sub._max_length > -1 && size > sub._max_length {
			_body_do_cbs(sub, nil, .Exceeds_Max_Size)
			return
		}

		sub._scanner.max_token_size = size
		sub._scanner.split          = scan_whatever
		sub._scanner.split_data     = sub
	...
	on_scan_chunk :: proc(sub: ^Has_Body) {
		sub._scanner.max_token_size = sub._max_length
		sub._scanner.split          = scan_lines

		on_scan_empty_line :: proc(sub: ^Has_Body, token: string, err: bufio.Scanner_Error) {
			if err != nil {
				_body_do_cbs(sub, nil, _bufio_error_to_body_error(err))
				return
			}
			...
			scanner_scan(sub._scanner, sub, on_scan)
		}

		scanner_scan(sub._scanner, sub, on_scan_empty_line)
	}
	...
	on_scan_trailer :: proc(sub: ^Has_Body, line: string, err: bufio.Scanner_Error) {
		...
		key, ok := header_parse(&sub.headers, string(line), allocator=sub.body_allocator)
		if !ok {
			log.infof("Invalid header when decoding chunked body: %q", string(line))
			_body_do_cbs(sub, nil, .Invalid_Trailing_Header)
			return
		}
