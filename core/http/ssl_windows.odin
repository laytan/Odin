...
// TODO: use kernel TLS (KTLS)?
// Should mean we can do the handshake here, then enable KTLS on the socket, and hand over to `nbio` procs.
// Whatever it sends should then be encrypted/decrypted in the kernel.
// Saves copies, and means we don't need the polling we do, it would just hook into overlapped IO.
...
native_ssl_implementation :: proc(allocator := context.allocator) -> SSL_Client {
	...
	return {
		...
		client_destroy = proc(_client: SSL_Client) {
			client := (^Schannel_Client)(_client.user_data)
		...
		connect = proc(_c: SSL_Connection) -> (res: SSL_Result, capabilities: SSL_Capabilities) {
			c := (^Schannel_Connection)(_c)
		...
		send = proc(_c: SSL_Connection, datas: [][]byte) -> (sent: int, res: SSL_Result) {
			c    := (^Schannel_Connection)(_c)
			if len(datas) > 1 {
				unimplemented()
			}
