...
assert_has_td :: #force_inline proc(loc := #caller_location) {
	assert(td != nil, "The thread you are calling from is not a server/handler thread", loc)
}
...
serve :: proc(s: ^Server, h: Handler) -> (err: net.Network_Error) {
	...
	sync.wait_group_add(&s.threads_closed, len(s.threads))
	for &server_thread in s.threads[1:] {
		server_thread.thread = thread.create_and_start_with_poly_data3(s, false, &server_thread, _server_thread_init, context)
	}

	_server_thread_init(s, true, &s.threads[0])
...
_server_thread_init :: proc(s: ^Server, main_thread := false, _td: ^Server_Thread) {
	td = _td
	...
	td.curr_accept = nbio.accept_poly(s.tcp_sock, s, on_accept)
...
server_shutdown :: proc(s: ^Server) {
	...
	for server_thread in s.threads {
		nbio.wake_up(server_thread.event_loop)
	}
}
...
on_accept :: proc(op: ^nbio.Operation, server: ^Server) {
	if op.accept.err != nil {
		...
		case .Insufficient_Resources:
			...
			nbio.timeout_poly(time.Second, server, proc(op: ^nbio.Operation, server: ^Server) {
				td.curr_accept = nbio.accept_poly(server.tcp_sock, server, on_accept)
			})
			return
		}
...
conn_handle_reqs :: proc(c: ^Connection) {
	scanner_recv :: proc(c: rawptr, buf: []byte, s: ^Scanner, callback: On_Scanner_Read) {
		...
		c.curr_recv = nbio.recv_poly2(c.socket, {buf}, c, callback, on_recv, timeout=s.timeout)

		on_recv :: proc(op: ^nbio.Operation, c: ^Connection, callback: On_Scanner_Read) {
...
conn_handle_req :: proc(c: ^Connection, allocator := context.temp_allocator) {
	...
	on_header_line :: proc(l: ^Loop, token: string, err: bufio.Scanner_Error) {
		...
		if _, ok := header_parse(&l.req.headers, token, clone_value=false, allocator=context.temp_allocator); !ok {
