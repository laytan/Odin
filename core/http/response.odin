...
response_send :: proc(r: ^Response, conn: ^Connection, loc := #caller_location) {
...

	{
		...
		if queue.len(conn.responses) == 1 {
			nbio.send(conn.socket, buf, conn, on_response_sent)
		} else {
...
@(private)
on_response_sent :: proc(op: ^nbio.Operation) {
	conn := cast(^Connection)op.user_data[0]

	res := queue.pop_front(&conn.responses)
	delete(res.buf, conn.server.conn_allocator)

	if op.send.err != nil {
		log.warnf("[%v] could not send response: %v", conn.socket, op.send.err)
	...
	if queue.len(conn.responses) > 0 {
		...
		nbio.send(conn.socket, queue.front(&conn.responses).buf, conn, on_response_sent)
		return
	}
