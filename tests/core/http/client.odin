...
@(test)
test_incoming_server_sent_events :: proc(t: ^testing.T) {

	Expected_Event :: struct {
		type:    string,
		data:    string,
		last_id: string,
		retry:   time.Duration,
...
`,
		...
		// NOTE: weird case where there is no message but there is something to parse.
		// This will be parsed again until there is actually a message after it like the next test case.
		{
			input = "retry: 123\n",
...
`,
	...
	for test, testi in cases {
		res := http.Incoming_Response{
			body = slice.clone_to_dynamic(transmute([]byte)test.input, context.temp_allocator),
		}

		it: http.Incoming_SSE_Iterator

		i := 0
		for type, data in http.incoming_SSE_iterator(&it, res.body[:]) {
			defer i += 1

			testing.expect(t, i < len(test.expected), "more events than expected")
			expected := test.expected[i]
			require_value(t, type, expected.type)
			require_value(t, data, expected.data, "test %v.%v: %q != %q", testi, i, data, expected.data)
			require_value(t, it.last_id, expected.last_id)
			require_value(t, it.retry, expected.retry)
		}

		require_value(t, it.retry, test.retry, "test %v: %v != %v", testi, it.retry, test.retry)
		http.incoming_SSE_iterator_destroy(it, &res.body)
		require_value(t, string(res.body[:]), test.left, "test %v: %q != %q", testi, string(res.body[:]), test.left)
	}
}
