...
gb_internal void init_build_context(TargetMetrics *cross_target, Subtarget subtarget) {
	...
	if (metrics->os == TargetOs_darwin) {
		if (!bc->minimum_os_version_string_given) {
			...
			// If the current user's system MacOS version is less than 11, lower the minimum version to that version.
			// TODO: check if ODIN_MINIMUM_OS_VERSION is correct.
			gbString sw_vers = gb_string_make(heap_allocator(), "");
			if (system_exec_command_line_app_output("sw_vers --productVersion", &sw_vers)) {
				uint32_t major = 0;
				if (sscanf(cast(const char *)sw_vers, "%u.%*u.%*u", &major)) {
					if (major < 11) {
						gb_string_clear(sw_vers);
						gb_string_append_fmt(sw_vers, "%d.0.0", major);
						bc->minimum_os_version_string = make_string_c(sw_vers);
					}
				}
			}
			#endif
